...
def encrypt_user_auth(exchange_dict, user_id):
    exchange_name = exchange_dict['name']
    current_app.logger.info(f'Encrypting User {user_id} exchange auth for {exchange_name}')
...
def upload_encrypted_auth(encrypted_text: bytes, user_id: int, exchange: str) -> Tuple[str, str]:
    current_app.logger.info(f'Uploading encrypted {exchange} auth for user {user_id}')
...
def upload_user_auth(exchange_dict, user_id):
