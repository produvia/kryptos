...
from catalyst.api import order, record, get_open_orders
...
def _handle_buy_sell_decision(context, data, signal, price):
    ...
    if context.position is None and context.asset in positions:
        ...
        context.position = dict(
            cost_basis=position["cost_basis"], amount=position["amount"], stop=None
        )
    ...
    if context.position is not None:
        ...
        if signal == "long":
            ...
            context.position = dict(cost_basis=price, amount=buy_amount, stop=None)
...
def trade_logic(context, data):
    ...
    if context.last_bar is None or (context.last_bar + timedelta(minutes=15)) <= dt:
        context.last_bar = dt
    else:
        return
