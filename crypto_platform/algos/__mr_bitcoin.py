...
def make_pipeline(context):
    return Pipeline(
        columns={
            ...
            "sma": SimpleMovingAverage(inputs=[CryptoPricing.close], window_length=context.WINDOW),
            "std": AnnualizedVolatility(
                inputs=[CryptoPricing.close], window_length=context.WINDOW, annualization_factor=1
            ),
        }
    )
...
def analyze(context=None, results=None):
    ...
    ax2.plot(
        buys.index, context.TICK_SIZE * results.price[buys.index], "^", markersize=10, color="g"
    )
    ax2.plot(
        sells.index, context.TICK_SIZE * results.price[sells.index], "v", markersize=10, color="r"
    )
    ...
    results[["algorithm", "benchmark"]].plot(ax=ax4)
