...
def dump_summary_table(algo, config, context, df):
    ...
    df_quant['backtest_days'] = (df.period_open.max() - df.period_open.min()).days\
                                + (((df.period_open.max() - df.period_open.min()).seconds)\
                                / (3600*24))
    df_quant['backtest_weeks'] = df_quant.backtest_days / 7

    ordered_trades = pd.DataFrame([(t['dt'], t['price'], t['amount'])
                                for items in df.transactions.values for t in items],
                                columns=['time', 'price', 'amount'])
    df_quant['number_of_trades'] = int(len(ordered_trades)/2)
    df_quant['average_trades_per_week_avg'] = float(df_quant.number_of_trades)\
                                            / float(df_quant.backtest_weeks)
    if int(df_quant.number_of_trades):
        df_quant['average_trade_amount_usd'] = (ordered_trades.amount * ordered_trades.price)\
                                                .abs().mean()\
                                                / float(df_quant.number_of_trades)
    else:
    ...
    df_quant['net_profit_pct'] = (df_quant.ending_capital - df_quant.initial_capital)\
    ...
    df_quant['max_daily_drawdown_pct'] = ((df.portfolio_value - df.portfolio_value.shift(1))\
                                        /df.portfolio_value.shift(1)).min() * 100
    df_quant['max_weekly_drawdown_pct'] = ((df.portfolio_value - df.portfolio_value.shift(7))\
                                        /df.portfolio_value.shift(7)).min() * 100
    # Note: removing first 30 samples of sharpe ratio
    df_quant['sharpe_ratio_avg'] = df.sharpe[30:].mean()
    df_quant['std_rolling_10_day_pct_avg'] = (df.portfolio_value.rolling(10).std()/df.portfolio_value).mean()
    df_quant['std_rolling_100_day_pct_avg'] = (df.portfolio_value.rolling(100).std()/df.portfolio_value).mean()
...
def dump_plots_to_file(algo, config, df):
    ...
    dump_metric_plot(pd.DataFrame([(t['dt'], t['amount'])
                                for t_list in df.transactions for
                                t in t_list])\
                        .set_index(0),
                    'Trades', SAVE_FOLDER)
    ...
    dump_metric_plot(df.portfolio_value.rolling(10).std() / df.portfolio_value,
                    'Standard Deviation % (10-day)', SAVE_FOLDER)
    dump_metric_plot(df.portfolio_value.rolling(100).std() / df.portfolio_value,
                    'Standard Deviation % (100-day)', SAVE_FOLDER)
    dump_metric_plot((df.portfolio_value - df.portfolio_value.shift(1))\
                        / df.portfolio_value.shift(1) * 100,
                    'Max Daily Drawdown %', SAVE_FOLDER)
    dump_metric_plot((df.portfolio_value - df.portfolio_value.shift(7))\
                        / df.portfolio_value.shift(7) * 100,
                    'Max Weekly Drawdown %', SAVE_FOLDER)

def dump_metric_plot(metric, metric_name, save_folder):
    plt.close()
    ax = metric.plot(legend=metric_name)
    f_name = metric_name.replace(' ', '_') + '.png'
    plt.savefig(os.path.join(save_folder, f_name),
